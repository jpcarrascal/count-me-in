if(experiment) // if experimental mode is on
{
  ...
  setTimeout(function(){
    ...
    if(isSynth > 0) {
      ...
      rotateMe.forEach(element => element.remove());
      console.log("Synth track detected");
    } else {
  ...
  document.querySelector("#prompt-submit").addEventListener('click', function(){
    if(prompt.value != "") {
      console.log("Prompt submitted: " + prompt.value);
